...
|---|---|---|
| First loss warning | ₹20,000 down | You get a warning message. Nothing is closed automatically. |
| Second loss warning | ₹30,000 down | A stronger warning, suggesting you cut your position size in half. Still nothing closed automatically. |
| Hard loss stop | ₹40,000 down | Everything (except protective hedges) gets closed automatically, right away. |
| Profit target | ₹80,000 up | Everything (except protective hedges) gets closed automatically to lock in the win. |
| Profit check-in step | Every ₹5,000 | You get an update every time your running total crosses another ₹5,000, in either direction, until the "trailing lock" below takes over. |
| Trailing lock starts | Once you're ₹40,000 up | From here on, it tracks your highest profit point of the day and protects a chunk of it — see section 3 for how much. |
| Early "green day" floor | Arms at ₹20,000 up, floor at ₹5,000 | A gentler safety net that only applies *before* the trailing lock above has kicked in. If profit reaches ₹20,000 then falls back to ₹5,000, everything closes. |
| Cool-off after a forced exit | 15 minutes | After any of the automatic closes above, if you open something new in the next 15 minutes, it gets automatically closed again too. |
| What counts as a "hedge" (ignored by all the above) | Priced under ₹5 | Cheap positions are assumed to be insurance, not real bets, so they're never touched by any auto-close rule. |
...
- **Loss warnings / hard loss stop** — Lower the ₹ amount and it reacts sooner (safer, but you'll get stopped out of a wobble more easily). Raise it and you're giving yourself more room to be wrong before anything is forced closed.
- **Profit target** — Lower it and you'll lock in wins earlier but leave more on the table on a good day. Raise it and you're letting winners run longer, at the risk of giving more of it back.
...
Exited: NIFTY2582624650CE, NIFTY2582624550PE
...
Both, depending on what you mean. The daily NIFTY trade and the weekly hedge are fully automatic — it decides the strikes, places the orders, and manages the stop-loss without you doing anything. For everything *else* you trade yourself, it never opens a new position on its own — it only protects what you've already opened (automatic stop-loss, and closing everything if a threshold is hit).
